...
export default function ColorPicker({
  ...
  const toggleDisplay = (position: number) => {
    setDisplays((prevState) => {
      return prevState.map((display: boolean, index: number) =>
        index === position ? !display : display
      );
    });
  };
          ...
          <Grid item>
            ...
            {displays[index] ? (
              <div className={classes.popover}>
                ...
                <ChromePicker
                  color={color}
                  onChange={(color) => updateColors(color.hex, index)}
                />
              </div>
            ) : null}
            {/* </ClickAwayListener> */}
